<template>
  <div class="weather-item">
    ...
    <div id="moonphase" v-if="props.astro">
      ...
      <div class="daylightmoduleposition">
        <div class="weather34sunlightday">
          {{ props.astro.day_length }}<weather34daylightdaycircle></weather34daylightdaycircle> hrs<br />Total Daylight
        </div>
        ...
        <div class="weather34sunriseday">
          ...
          >
            ...
            ></path></svg
          >Sun Rise<br />{{ todayTomorrow('sunrise') }} {{ props.astro.sunrise
        ...
        <div class="weather34sunsetday">
          ...
          >
            ...
            ></path></svg
          >Sun Set<br />{{ todayTomorrow('sunset') }} {{ props.astro.sunset
        ...
        <div class="sundialcontainerdiv2">
          <div id="sundialcontainer" class="sundialcontainer">
            <canvas id="sundial" class="suncanvasstyle" v-sundial="props.astro"></canvas>
...
function hoursTilSunSet() {
  let sunsetTime = moment(props.astro.date + ' ' + props.astro.sunset + ':00');
  let now = moment();
  let h = moment.duration(sunsetTime.diff(now)).hours();
  if (h < 1 && hasSunset) {
    sunsetTime = moment(props.astro.tomorrow.date + ' ' + props.astro.tomorrow.sunrise + ':00');
  }
...
function todayTomorrow(type) {
  let event = setDateTime(props.astro[type]);
  if (moment() > event) {
    return 'Tomorrow';
  } else {
    return 'Today';
...
function darkness() {
  ...
  let hours = day.subtract(timeToDecimal(props.astro.day_length), 'hours');
...
function sunColor() {
  let value = '';
  if (props.astro.sun_altitude <= 0.5 && props.astro.sun_altitude > -4) {
    value = 'rgba(255, 112, 50, 0.5)';
  } else if (props.astro.sun_altitude <= 0) {
