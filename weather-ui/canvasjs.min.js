...
m||D)p._creditLink.setAttribute("href",p.creditHref),p._creditLink.innerHTML=p.creditText;p._creditLink&&p.creditHref&&p.creditText?(p._creditLink.parentElement||p._canvasJSContainer.appendChild(p._creditLink),p._creditLink.style.top=p.height-14+"px"):p._creditLink.parentElement&&p._canvasJSContainer.removeChild(p._creditLink)}}}function ta(p,m){Ja&&(this.canvasCount|=0,window.
console.log(++this.canvasCount));var D=document.createElement("canvas");D.setAttribute("class","canvasjs-chart-canvas");Oa(D,
