              ...
              <div class="fsdarkskydiv" v-for="day in forecast.daily.data" v-bind:key="day.time">
                <value>
                  <div class="darkskyforecastinghome">
                    ...
                    <darkskytemphihome>
                      <span>
                        <temp :class="tempColor(day.temperatureHigh)"
                          >{{ day.temperatureHigh }}°</temp
                        >
                        <grey> | </grey>
                      </span>
                    </darkskytemphihome>
                    <darkskytemplohome>
                      <span>
                        <temp :class="tempColor(day.temperatureLow)"
                          >{{ day.temperatureLow }}°</temp
                        >
                      </span>
                    </darkskytemplohome>
                    <darkskytemplohome>
                    ...
                    <darkskytempwindhome>
                      <span>{{ day.summary }}</span> </darkskytempwindhome
                    ><br />&nbsp;<br />
                    <darkskypreciphome v-if="day.precipType == 'snow'">
                      <span
                        >Snow <blue1>{{ day.precipAccumulation }}</blue1> in
                        <blue1>{{ day.precipProbability }}</blue1
                        >%</span
                      >
                    </darkskypreciphome>
                    <darkskypreciphome v-if="day.precipType == 'rain'">
                      <span
                        >Rain <blue1>&nbsp; {{ day.precipIntensity }}</blue1> in
                        <blue1>{{ day.precipProbability }}</blue1
                        >%</span
                      >
                    </darkskypreciphome>
...
.darkskyforecastinghome {
  ...
  border-top: 1px solid rgba(97, 106, 114, 1);
}
...
.redtds {
  background: -webkit-linear-gradient(90deg, #d86858, rgba(211, 93, 78, 0.7));
  background: linear-gradient(90deg, #d86858, rgba(211, 93, 78, 0.7));
}

.purpletds {
  background: -webkit-linear-gradient(90deg, #d86858, rgba(157, 59, 165, 0.4));
  background: linear-gradient(90deg, #d86858, rgba(157, 59, 165, 0.4));
}
