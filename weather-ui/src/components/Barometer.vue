  ...
  <div class="weather-item">
    ...
    <div id="barometer" v-if="minmax && current">
      ...
      <div class="barometermax">
        <div class="barometerorange">
          <valuetext>Max ({{ minmax.max.day.date | timeFormat}})<br>
            <maxred>
              <value>{{ minmax.max.day.value}}</value>
            </maxred> inHg
          </valuetext>
        </div>
      </div>
      <div class="barometermin">
        <div class="barometerblue">
          <valuetext>Min ({{ minmax.min.day.date | timeFormat}})<br>
            <minblue>
              <value>{{ minmax.min.day.value }}</value>
            </minblue>&nbsp;inHg
          </valuetext>
        </div>
      </div>
      ...
      <div class="barometerconverter">
        <div class="barometerconvertercircleblue">{{ inToMMHG.toFixed(2) }}<smallrainunit>hPa</smallrainunit>
        </div>
      </div>
...
export default {
  ...
  mounted() {
       axios.get('/api/trend/barometer').then(response => (this.trend = response.data))
       axios.get('/api/minmax/baromrelin').then(response => (this.minmax = response.data));
  },
  methods: {},
  filters: {
    now: function(date) {
