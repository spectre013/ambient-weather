<template>
  <div class="weather-item">
    ...
    <span class="moduletitle"
      >Direction | Wind (<valuetitleunit>{{ windLabel }}</valuetitleunit
      >)</span
    ><br />
    <div id="windspeed" v-if="wind && current">
      ...
      <div class="windspeedvalues">
        <div class="windspeedvalue">
          {{ current.windspeedmph | windDisplay($store.getters.speed) }}
          <div class="windunitidspeed">Currently</div>
          <div class="windunitspeed">{{ windLabel }}</div>
        </div>
        <div class="windgustvalue">
          {{ current.windgustmph | windDisplay($store.getters.speed) }}
          <div class="windunitgust">{{ windLabel }}</div>
      ...
      <div class="windspeedtrend1">
        <valuetext
          ...
          <max>
            <value>
              <maxred>{{ wind.gust.value | windDisplay($store.getters.speed) }}</maxred>
            </value></max
          >
          <supmb> {{ windLabel }}</supmb
          ><br />
          Gust ({{ wind.gust.date | timeFormat }})
        </valuetext>
      </div>
      ...
      <div class="homeweathercompass1">
        <div class="homeweathercompass-line1">
          <div
            class="thearrow2"
            v-bind:style="{ transform: 'rotate(' + current.winddir + 'deg)' }"
          ></div>
        ...
        <div class="windirectiontext1">
          <span> {{ current.winddir | degToCompass }}<br /></span>
        </div>
      </div>
      <div class="weather34windrun">
        ...
        ><grey>
          <valuetext1
            >&nbsp;{{ windRun() | windDisplay($store.getters.speed) }}
            <grey><weather34windrunspan>mi</weather34windrunspan></grey></valuetext1
          ></grey
        >
      </div>
      <br />
