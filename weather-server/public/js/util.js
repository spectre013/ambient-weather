...
function rainDisplay(rn, units ) {
    if (units === 'metric') {
        return (rn * 25.4).toFixed(0);
    }
    return rn.toFixed(2);
}
function baroDisplay(baro, units) {
    const b = baro;
    if (units === 'metric') {
...
function windDisplay(wind, units) {
    if (units === 'metric') {
        return (wind / 2.237).toFixed(0);
    }
    return wind.toFixed(0);
}
function tempDisplay(temp, units) {
    let t = temp;
    if (units === 'metric') {
        t = ((temp - 32) * 5) / 9;
    }
 ...
 */
function distanceDisplay(value, units)  {
 ...
 */
function tempToHex(tempFahrenheit)  {
 ...
 */
function tempColor(tempFahrenheit) {
    if (tempFahrenheit >= 90) {
...
function dewPointClass(dewpoint) {
    if (dewpoint > 69.8) {
...
function humidityClass(humidity) {
    if (humidity > 90) {
...
function temperaturetoday(temp) {
    if (temp >= 105.8) {
 ...
 */
function BeaufortHex(windSpeed) {
    if (windSpeed < 1) {
...
function windClass(beaufortValue)  {
    if (beaufortValue >= 12) {
        return 'beaufort6';
...
function getTemperatureGradient(temp1, temp2) {
...
function windRun(value) {
    return value * moment().hours();
}
...
function BeaufortSvg(windspeed)  {
    return beaufortScale(windspeed).svg;
}
function beaufortText(windspeed) {
    return beaufortScale(windspeed).text;
}
function beaufortClass(windspeed) {
...
function showForecast(event) {
    const element = document.getElementById('summary-' + event.id);
    element.classList.toggle('hidden');
}

// function createDial(speed, direction, gusts, color, radiusColor, tickColor)   {
//     const windData = {
//         speed: speed,
