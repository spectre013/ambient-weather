...
export default function WindCard({ wind, hours, units = "F" }) {
    ...
    const u = wUnit(units);
    return (
        <Card label="Wind" right={`gust ${showW(wind.maxdailygust, units).toFixed(0)} ${u} today`}>
            <div className="compass-wrap">
                ...
                <div className="compass-readout">
                    ...
                    <div className="row"><span>Day max</span><span>[{fmtTime(wind.gustminmax.max.day.date)}] {showW(wind.maxdailygust, units).toFixed(1)} {u}</span></div>
                </div>
            </div>
